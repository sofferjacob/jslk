...
heap_t *createHeap(uint32_t start, uint32_t end_addr, uint32_t max, uint8_t supervisor, uint8_t readonly)
{
    ...
    heap->index = placeOrderedArray( (void*)start, HEAP_INDEX_SIZE, &header_t_less_than);
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    header_t *orig_hole_header = (header_t *)lookupOrderedArray(iterator, &heap->index);
    ...
    if (orig_hole_size - new_size > 0)
    {
        ...
        insertOrderedArray(&heap->index, (void *)hole_header);
    }
...
void free(void *p, heap_t *heap)
{
        ...
        test_header->is_hole)
    {
        ...
        uint32_t iterator = 0;
        while ( (iterator < heap->index.size) &&
                (lookupOrderedArray(iterator, &heap->index) != (void*)test_header) )
            iterator++;
